...
function ListingDetailScreen() {
  ...
  const showBuyBar = !isOwnListing && !soldOut;
  ...
  return (
    <div className="animate-fade-in space-y-4 pb-28 md:pb-4">
      <button
        type="button"
      ...
      {/* Mobile: stacked · Desktop: media | details (BRD 12.3) */}
      <div className="grid gap-6 lg:grid-cols-2 lg:items-start">
        <div className="overflow-hidden rounded-2xl border border-border bg-card">
          <div className="relative aspect-[4/3] bg-muted lg:aspect-square">
            {images.length > 0 ? (
              <img
          ...
          {images.length > 1 && (
            <fieldset className="flex gap-2 overflow-x-auto p-2">
              ...
              {images.map((image, i) => (
                <button
                  ...
                  className={`h-14 w-14 shrink-0 overflow-hidden rounded-lg border-2 ${imageIndex === i ? "border-primary" : "border-transparent"}`}
                >
                  <img
                    src={image.url}
                    alt=""
        ...
        <div className="space-y-4">
          <div className="space-y-3">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <h1 className="font-display text-xl font-bold leading-snug md:text-2xl">
                  {data.title}
                </h1>
              </div>
              <p className="whitespace-nowrap font-mono text-xl font-bold text-primary md:text-2xl">
          ...
          {/* Desktop buy panel */}
          {showBuyBar && (
            <div className="hidden items-center gap-3 rounded-2xl border border-border bg-card p-4 lg:flex">
              {quantityControl}
              <Button
                onClick={handleBuy}
